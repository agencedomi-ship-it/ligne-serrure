...
async function rechercherSuggestions(query, limit) {
    ...
    if (/^\d{5}$/.test(queryClean)) {
        ...
        if (villesCP.length > 0) return villesCP.slice(0, limit);
        return [];
    }
                        ...
                        if (v.codesPostaux) {
                            for (var j = 0; j < v.codesPostaux.length; j++) {
                                ...
                                if (cp.indexOf(queryClean) === 0) {
                                    var exists = suggestions.some(function(s) { return s.slug === slugify(v.nom); });
                                    if (!exists) suggestions.push({ type: "ville", nom: v.nom + " (" + cp + ")", slug: slugify(v.nom) });
                                }
                            }
                        }
                    }
        ...
        try {
            ...
            if (response.ok) {
                ...
                for (var i = 0; i < villes.length && suggestions.length < limit; i++) {
                    ...
                    if (DEPARTEMENTS_ACTIFS[v.codeDepartement]) {
                        var exists = suggestions.some(function(s) { return s.slug === slugify(v.nom); });
                        if (!exists) suggestions.push({ type: "ville", nom: v.nom + " (" + v.codeDepartement + ")", slug: slugify(v.nom) });
                    }
                }
            }
        } catch (e) {}
...
export async function onRequest(context) {
    ...
    var headers = { "Content-Type": "application/json", "Access-Control-Allow-Origin": "*", "Cache-Control": "public, max-age=300" };
    if (query.length < 2) return new Response(JSON.stringify([]), { headers: headers });
    try {
