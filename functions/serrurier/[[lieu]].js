...
function generateSOSHTML(nom, titreComplet) {
    ...
    html += '.urgence-bar{background:var(--red);padding:12px 0;position:relative;z-index:10}';
    ...
    html += '.status-badge{display:inline-flex;align-items:center;gap:8px;background:rgba(34,197,94,0.2);border:1px solid var(--green);padding:10px 20px;border-radius:50px;margin-bottom:20px}';
    html += '.status-badge .dot{width:10px;height:10px;background:var(--green);border-radius:50%;animation:dotPulse 1.5s infinite}';
    html += '.status-badge span{font-size:14px;font-weight:600;color:var(--green)}';
    ...
    html += '.hero{background:linear-gradient(rgba(0,0,0,0.6),rgba(0,0,0,0.6)),url("/images/serrurier-entreprise.jpg") center/cover no-repeat;padding:80px 0;position:relative;z-index:1}';
    ...
    html += '.hero h1{font-size:48px;line-height:1.1;margin-bottom:20px;color:var(--white)}';
    html += '.hero h1 span{color:var(--red)}';
    html += '.hero-subtitle{font-size:18px;color:rgba(255,255,255,0.9);margin-bottom:25px;line-height:1.6}';
    ...
    // Services mobile first avec image en fond
    html += '.services{padding:40px 0;background:var(--gray-light);position:relative;z-index:1}';
    html += '.container{max-width:1200px;margin:0 auto;padding:0 20px}';
    html += '.section-header{text-align:center;margin-bottom:30px}';
    html += '.section-header h2{font-size:28px;color:var(--dark);margin-bottom:10px}';
    html += '.section-header h2 span{color:var(--red)}';
    html += '.section-header p{font-size:15px;color:var(--gray)}';
    html += '.services-grid{display:grid;grid-template-columns:1fr;gap:20px}';
    html += '.service-card{position:relative;height:280px;border-radius:16px;overflow:hidden;cursor:pointer}';
    html += '.service-card .service-img{position:absolute;top:0;left:0;width:100%;height:100%;object-fit:cover;transition:transform 0.5s ease}';
    html += '.service-card:hover .service-img{transform:scale(1.1)}';
    html += '.service-card::after{content:"";position:absolute;top:0;left:0;width:100%;height:100%;background:linear-gradient(to top,rgba(0,0,0,0.85) 0%,rgba(0,0,0,0.3) 50%,rgba(0,0,0,0.1) 100%);z-index:1}';
    html += '.service-card::before{content:"";position:absolute;top:0;left:0;width:0;height:4px;background:linear-gradient(90deg,var(--red),#FF6B6B,var(--red));transition:width 0.4s ease;z-index:10}';
    html += '.service-card:hover::before{width:100%}';
    html += '.service-content{position:absolute;bottom:0;left:0;right:0;padding:25px;z-index:2;text-align:left}';
    html += '.service-card h3{font-size:20px;color:var(--white);margin-bottom:8px;text-transform:uppercase}';
    html += '.service-card p{color:rgba(255,255,255,0.8);font-size:14px;line-height:1.5;margin-bottom:15px}';
    html += '.service-price{display:inline-block;background:var(--red);color:var(--white);padding:8px 20px;border-radius:50px;font-weight:700;font-size:14px}';
    html += '@media(min-width:600px){.services-grid{grid-template-columns:repeat(2,1fr)}.section-header h2{font-size:34px}.service-card{height:300px}}';
    html += '@media(min-width:1024px){.services{padding:80px 0}.services-grid{grid-template-columns:repeat(3,1fr);gap:25px}.section-header{margin-bottom:50px}.section-header h2{font-size:40px}.service-card{height:320px}.service-card h3{font-size:22px}}';
    ...
    html += 'footer a{color:rgba(255,255,255,0.7);text-decoration:none;margin:0 10px;transition:color 0.3s}';
    html += 'footer a:hover{color:var(--white)}';
    html += '.footer-links{margin-top:15px}';
    ...
    html += '@media(max-width:1024px){.hero-inner,.contact-grid{grid-template-columns:1fr}.avis-grid{grid-template-columns:repeat(2,1fr)}.hero{padding:40px 0}}';
    html += '@media(max-width:768px){.hero h1{font-size:32px}.section-header h2{font-size:28px}.avis-grid{grid-template-columns:1fr}.form-row{grid-template-columns:1fr}.header-inner{flex-direction:column;gap:15px}.header-phone{width:100%;justify-content:center}.hero-features{justify-content:center}.hero-buttons{flex-direction:column;width:100%}.hero-buttons a{width:100%;justify-content:center}}';
    ...
    html += '<div class="urgence-bar"></div>';
    ...
    html += '<section class="hero"><div class="hero-inner"><div class="hero-content"><div class="status-badge"><span class="dot"></span><span>Serrurier disponible maintenant</span></div><h1>SERRURIER <span>' + nom.toUpperCase() + '</span></h1><p class="hero-subtitle">Intervention URGENTE en 15 minutes ' + titreComplet + '. Ouverture de porte, changement de serrure, depannage 24h/24.</p><div class="hero-features"><div class="hero-feature"><svg><use href="#i-bolt"/></svg> 15 min chrono</div><div class="hero-feature"><svg><use href="#i-shield"/></svg> Agree assurance</div><div class="hero-feature"><svg><use href="#i-euro"/></svg> Devis gratuit</div><div class="hero-feature"><svg><use href="#i-clock"/></svg> 24h/24</div></div><div class="hero-buttons"><a href="tel:' + TEL_CLEAN + '" class="btn-call"><svg><use href="#i-phone"/></svg> APPELER MAINTENANT</a><a href="#contact" class="btn-devis">Devis gratuit</a></div></div>';
    
    html += '<div class="express-form"><div id="expressFormContainer"><h3>INTERVENTION EXPRESS</h3><p class="subtitle">Rappel en moins de 2 minutes</p><form id="expressForm"><div class="form-group"><label>Nom *</label><input type="text" id="exp_nom" required placeholder="Votre nom" minlength="2"></div><div class="form-group"><label>Code Postal *</label><input type="text" id="exp_cp" required placeholder="75001" pattern="[0-9]{5}" maxlength="5" title="5 chiffres requis"></div><div class="form-group"><label>Telephone *</label><input type="tel" id="exp_tel" required placeholder="0612345678" pattern="[0-9]{10}" maxlength="10" title="10 chiffres requis"></div><button type="submit" class="btn-express">DEMANDER UN RAPPEL</button></form></div><div class="success" id="expressSuccess"><svg><use href="#i-check"/></svg><h4>Demande envoyee !</h4><p>Un serrurier vous rappelle dans 2 minutes.</p></div></div>';
    html += '</div></section>';
    
    // Assurances - noms corrects
    html += '<section class="assurances"><div class="assurances-inner"><div class="assurances-title">Agree par toutes les assurances</div><div class="assurances-logos"><img src="/images/axa.png" alt="AXA"><img src="/images/maif.png" alt="MAIF"><img src="/images/groupama.png" alt="Groupama"><img src="/images/Allianz.png" alt="Allianz"><img src="/images/générali.png" alt="Generali"></div></div></section>';
    
    // Services - noms corrects avec espaces
    ...
    html += '<div class="service-card"><img src="/images/changement de serrure.jpg" alt="Changement serrure" class="service-img"><div class="service-content"><h3>CHANGEMENT SERRURE</h3><p>Remplacement rapide toutes marques. Serrures 3 et 5 points.</p><span class="service-price">Des 129 EUR</span></div></div>';
    html += '<div class="service-card"><img src="/images/blindage de porte.jpeg" alt="Blindage porte" class="service-img"><div class="service-content"><h3>BLINDAGE PORTE</h3><p>Securisation complete avec solutions certifiees A2P.</p><span class="service-price">Des 890 EUR</span></div></div>';
    html += '<div class="service-card"><img src="/images/rideaux métalliques.jpg" alt="Rideau metallique" class="service-img"><div class="service-content"><h3>RIDEAU METALLIQUE</h3><p>Installation, reparation et depannage de rideaux metalliques.</p><span class="service-price">Des 150 EUR</span></div></div>';
    html += '<div class="service-card"><img src="/images/coffre fort.jpg" alt="Coffre fort" class="service-img"><div class="service-content"><h3>COFFRE-FORT</h3><p>Ouverture, installation et depannage de coffres-forts.</p><span class="service-price">Des 200 EUR</span></div></div>';
    html += '<div class="service-card"><img src="/images/serrurier-entreprise.jpg" alt="Porte blindee" class="service-img"><div class="service-content"><h3>PORTE BLINDEE</h3><p>Installation de portes blindees haute securite.</p><span class="service-price">Sur devis</span></div></div>';
    ...
    html += '<div class="faq-card"><h4>Quel est le delai d\'intervention ?</h4><p>Nos serruriers interviennent en moyenne en 15 a 30 minutes sur ' + nom + ' et ses environs.</p></div>';
    html += '<div class="faq-card"><h4>Intervenez-vous la nuit et le week-end ?</h4><p>Oui, notre service est disponible 24h/24 et 7j/7, y compris les jours feries.</p></div>';
    html += '<div class="faq-card"><h4>Comment se passe le paiement ?</h4><p>Nous acceptons les paiements par carte bancaire, especes et cheque.</p></div>';
    html += '<div class="faq-card"><h4>Vos serruriers sont-ils qualifies ?</h4><p>Tous nos techniciens sont certifies et experimentes.</p></div>';
    html += '<div class="faq-card"><h4>Proposez-vous une garantie ?</h4><p>Oui, toutes nos interventions sont garanties.</p></div>';
    html += '<div class="faq-card"><h4>Puis-je avoir un devis avant intervention ?</h4><p>Absolument. Nos serruriers etablissent un devis gratuit sur place.</p></div>';
    html += '<div class="faq-card"><h4>Quel est le delai d\'intervention ?</h4><p>Nos serruriers interviennent en moyenne en 15 a 30 minutes sur ' + nom + ' et ses environs.</p></div>';
    html += '<div class="faq-card"><h4>Intervenez-vous la nuit et le week-end ?</h4><p>Oui, notre service est disponible 24h/24 et 7j/7, y compris les jours feries.</p></div>';
    html += '</div></section>';
    
    html += '<footer><div class="container"><p>2025 ' + MARQUE + ' - Serrurier ' + nom + ' - Tous droits reserves</p><p class="footer-links"><a href="/mentions-legales">Mentions legales</a><a href="/confidentialite">Confidentialite</a><a href="/cgu">CGU</a></p></div></footer>';
    ...
    html += 'var TELEGRAM_CHAT="-4875894499";';
    html += 'var LIEU="' + nom + '";';
    html += 'var MARQUE="' + MARQUE + '";';
    
    html += 'function sendTelegram(msg){fetch("https://api.telegram.org/bot"+TELEGRAM_BOT+"/sendMessage",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({chat_id:TELEGRAM_CHAT,text:msg,parse_mode:"HTML"})});}';
    
    html += 'document.getElementById("expressForm").addEventListener("submit",function(e){e.preventDefault();var n=document.getElementById("exp_nom").value;var c=document.getElementById("exp_cp").value;var t=document.getElementById("exp_tel").value;var msg="🚨 <b>URGENCE EXPRESS</b>"+String.fromCharCode(10)+String.fromCharCode(10)+"<b>Nom:</b> "+n+String.fromCharCode(10)+"<b>CP:</b> "+c+String.fromCharCode(10)+"<b>Tel:</b> "+t+String.fromCharCode(10)+"<b>Page:</b> Serrurier "+LIEU;sendTelegram(msg);window.location.href="/merci";});';
    
    html += 'document.getElementById("contactForm").addEventListener("submit",function(e){e.preventDefault();var d={nom:document.getElementById("nom").value,telephone:document.getElementById("telephone").value,codepostal:document.getElementById("codepostal").value,adresse:document.getElementById("adresse").value,probleme:document.getElementById("probleme").value,urgence:document.getElementById("urgence").value};var msg="📋 <b>DEMANDE DE DEVIS</b>"+String.fromCharCode(10)+String.fromCharCode(10)+"<b>Nom:</b> "+d.nom+String.fromCharCode(10)+"<b>Tel:</b> "+d.telephone+String.fromCharCode(10)+"<b>CP:</b> "+d.codepostal+String.fromCharCode(10)+"<b>Adresse:</b> "+d.adresse+String.fromCharCode(10)+"<b>Probleme:</b> "+d.probleme+String.fromCharCode(10)+"<b>Urgence:</b> "+d.urgence+String.fromCharCode(10)+"<b>Page:</b> Serrurier "+LIEU;sendTelegram(msg);window.location.href="/merci";});';
...
export async function onRequest(context) {
    ...
    var html = generateHTML(lieu, data);
