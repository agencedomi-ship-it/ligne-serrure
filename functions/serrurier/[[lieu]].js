...
var TELEGRAM = { botToken: "8509813114:AAH4_U355i-wWokzi3WF2-JJX_PELV878I0", chatId: "7695629846" };
...
async function validerLieu(recherche) {
    ...
    try {
        var response = await fetch("https://geo.api.gouv.fr/communes?nom=" + encodeURIComponent(rechercheClean) + "&fields=nom,code,codeDepartement&limit=5");
        if (response.ok) {
...
function generateSOSHTML(nom, titreComplet) {
    ...
    html += '.urgence-bar{background:var(--dark);color:var(--white);padding:12px 0;position:relative;z-index:10}';
    ...
    html += '.urgence-bar .status{display:flex;align-items:center;gap:8px;background:rgba(34,197,94,0.2);padding:8px 16px;border-radius:50px;border:1px solid var(--green)}';
    html += '.urgence-bar .dot{width:10px;height:10px;background:var(--green);border-radius:50%;animation:dotPulse 1.5s infinite}';
    html += '.urgence-bar .status span{font-size:14px;font-weight:600;color:var(--green)}';
    ...
    html += '.express-form input:focus{outline:none;border-color:var(--green);background:var(--white)}';
    ...
    html += '.express-form .success svg{width:50px;height:50px;fill:var(--green);margin-bottom:15px}';
    ...
    html += '.avis-avatar{width:45px;height:45px;background:var(--green);color:var(--white);border-radius:50%;display:flex;align-items:center;justify-content:center;font-weight:700}';
    ...
    html += '.contact-method svg{width:30px;height:30px;fill:var(--green)}';
    html += '.contact-method h4{font-family:"Open Sans",sans-serif;font-size:13px;font-weight:600;color:var(--gray);margin-bottom:3px}';
    html += '.contact-method a{color:var(--green);font-size:18px;font-weight:700;text-decoration:none}';
    ...
    html += '.form-group input:focus,.form-group select:focus{outline:none;border-color:var(--green);background:var(--white)}';
    ...
    html += '.success-icon{width:70px;height:70px;background:var(--green);border-radius:50%;display:flex;align-items:center;justify-content:center;margin:0 auto 20px}';
    ...
    // JavaScript
    html += '<script>';
    html += 'var TELEGRAM_BOT="8509813114:AAH4_U355i-wWokzi3WF2-JJX_PELV878I0";';
    ...
    html += 'function sendTelegram(msg){fetch("https://api.telegram.org/bot"+TELEGRAM_BOT+"/sendMessage",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({chat_id:TELEGRAM_CHAT,text:msg,parse_mode:"HTML"})}).catch(function(e){console.log(e)});}';
    
    // Formulaire Express
    html += 'document.getElementById("expressForm").addEventListener("submit",function(e){e.preventDefault();var n=document.getElementById("exp_nom").value;var c=document.getElementById("exp_cp").value;var t=document.getElementById("exp_tel").value;var msg="🚨 <b>URGENCE EXPRESS</b>\\n\\n<b>Nom:</b> "+n+"\\n<b>CP:</b> "+c+"\\n<b>Tel:</b> "+t+"\\n<b>Page:</b> Serrurier "+LIEU;sendTelegram(msg);document.getElementById("expressFormContainer").style.display="none";document.getElementById("expressSuccess").classList.add("show");});';
