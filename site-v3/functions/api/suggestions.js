...
async function rechercherSuggestions(query, limit) {
    ...
    // 1. CODE POSTAL COMPLET (5 chiffres) - retourne toutes les villes
    ...
    // 2. DEBUT CODE POSTAL (2-4 chiffres)
    if (/^\d{2,4}$/.test(queryClean)) {
        ...
        if (DEPARTEMENTS_ACTIFS[codeDep]) {
            if (queryClean.length === 2) {
                suggestions.push({ 
                    type: 'departement', 
                    nom: DEPARTEMENTS_ACTIFS[codeDep] + ' (' + codeDep + ')', 
                    slug: codeDep 
                });
            }
            var villes = await getVillesDepartement(codeDep);
