...
async function validerLieu(recherche) {
    ...
    for (var code in DEPARTEMENTS_ACTIFS) {
        ...
        if (slugify(nom) === slug) {
            return { valide: true, type: 'departement', code: code, nom: nom, slug: code };
        }
    }
    ...
    for (var regionSlug in REGIONS) {
        ...
        if (slug === regionSlug || slug === slugify(regionData.nom)) {
            var depActifs = [];
            for (var i = 0; i < regionData.departements.length; i++) {
                if (DEPARTEMENTS_ACTIFS[regionData.departements[i]]) {
                    depActifs.push(regionData.departements[i]);
                }
            }
    ...
    if (/^\d{5}$/.test(rechercheClean)) {
        ...
        if (DEPARTEMENTS_ACTIFS[codeDep]) {
            ...
            for (var i = 0; i < villes.length; i++) {
                var v = villes[i];
                if (v.codesPostaux && v.codesPostaux.indexOf(rechercheClean) !== -1) {
                    return { valide: true, type: 'code_postal', code: rechercheClean, nom: v.nom, slug: rechercheClean, departement: codeDep };
                }
            }
        }
    ...
    for (var codeDep in DEPARTEMENTS_ACTIFS) {
        ...
        for (var i = 0; i < villes.length; i++) {
            var v = villes[i];
            if (slugify(v.nom) === slug) {
                return { valide: true, type: 'ville', code: v.code, nom: v.nom, slug: slugify(v.nom), departement: codeDep };
            }
        }
    }
...
<head>\
    ...
    <style>\
        ...
        *{margin:0;padding:0;box-sizing:border-box}\
        ...
        @media(max-width:1024px){.hero-inner{grid-template-columns:1fr}.trust-box{max-width:500px;margin:0 auto}.why-grid{grid-template-columns:repeat(2,1fr)}.contact-grid{grid-template-columns:1fr}.footer-grid{grid-template-columns:repeat(2,1fr)}}\
...
<body>\
    ...
    <section class="services" id="services">\
        <div class="container">\
            ...
            <div class="services-grid">\
                <div class="service-card"><div class="service-icon"><svg><use href="#i-door"/></svg></div><h3>Ouverture de porte</h3><p>Porte claquee, cle perdue ou cassee. Ouverture sans degat en 30 minutes.</p><span class="service-price">A partir de 89 EUR</span></div>\
                <div class="service-card"><div class="service-icon"><svg><use href="#i-lock"/></svg></div><h3>Changement de serrure</h3><p>Remplacement de serrure toutes marques. Serrures 3 et 5 points.</p><span class="service-price">A partir de 129 EUR</span></div>\
                <div class="service-card"><div class="service-icon"><svg><use href="#i-shield"/></svg></div><h3>Blindage de porte</h3><p>Securisez votre domicile avec nos solutions certifiees A2P.</p><span class="service-price">A partir de 890 EUR</span></div>\
                <div class="service-card"><div class="service-icon"><svg><use href="#i-key"/></svg></div><h3>Double de cles</h3><p>Reproduction de cles classiques et haute securite sur place.</p><span class="service-price">A partir de 15 EUR</span></div>\
                <div class="service-card"><div class="service-icon"><svg><use href="#i-home"/></svg></div><h3>Installation serrure</h3><p>Pose de serrures connectees, biometriques et haute securite.</p><span class="service-price">A partir de 149 EUR</span></div>\
                <div class="service-card"><div class="service-icon"><svg><use href="#i-tool"/></svg></div><h3>Depannage urgent</h3><p>Intervention 24h/24 pour toutes urgences serrurerie.</p><span class="service-price">A partir de 99 EUR</span></div>\
            </div>\
        </div>\
    </section>\
    ...
    <footer>\
        <div class="container">\
            <div class="footer-grid">\
                <div class="footer-brand"><h3>Ligne Serrure</h3><p>Votre serrurier de confiance ' + titreComplet + ', disponible 24h/24 et 7j/7.</p></div>\
                ...
                <div class="footer-links"><h4>Infos</h4><ul><li><a href="#">Tarifs</a></li><li><a href="#contact">Contact</a></li></ul></div>\
    ...
    <script>\
        var CALLMEBOT_PHONE = "' + CALLMEBOT.phone + '";\
        var CALLMEBOT_APIKEY = "' + CALLMEBOT.apikey + '";\
        var LIEU = "' + nom + '";\
        document.getElementById("contactForm").addEventListener("submit", function(e) {\
            ...
            btn.disabled = true;\
            btn.innerHTML = "Envoi...";\
            var data = {nom: document.getElementById("nom").value, telephone: document.getElementById("telephone").value, adresse: document.getElementById("adresse").value, probleme: document.getElementById("probleme").value, urgence: document.getElementById("urgence").value, message: document.getElementById("message").value || "Aucun"};\
            var msg = "*NOUVEAU CONTACT*\\n\\n*Page:* Serrurier " + LIEU + "\\n*Nom:* " + data.nom + "\\n*Tel:* " + data.telephone + "\\n*Adresse:* " + data.adresse + "\\n*Probleme:* " + data.probleme + "\\n*Urgence:* " + data.urgence + "\\n*Message:* " + data.message;\
            try { fetch("https://api.callmebot.com/whatsapp.php?phone=" + CALLMEBOT_PHONE + "&text=" + encodeURIComponent(msg) + "&apikey=" + CALLMEBOT_APIKEY, {method:"GET",mode:"no-cors"}); } catch(e) {}\
